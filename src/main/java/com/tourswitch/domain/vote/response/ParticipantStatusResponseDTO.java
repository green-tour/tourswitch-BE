...
public record ParticipantStatusResponseDTO(Long memberId, boolean completed, boolean extraCompleted,
                                            LocalDateTime completedAt) {

    public static ParticipantStatusResponseDTO from(ParticipantCompletionRow row) {
        return new ParticipantStatusResponseDTO(row.memberId(), row.completed(), row.extraCompleted(),
                row.completedAt());
    }
}
